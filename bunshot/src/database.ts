...
export async function insertDeploymentPreview(
  ...
  try {
    ...
    const checkResult = await client.query(
      'SELECT * FROM deployment_previews WHERE site_id = $1 AND deployment_id = $2 AND preview_path = $3 AND favicon_path = $4',
      [siteId, deploymentId, filePath, faviconPath]
    );
    ...
    const result = await client.query(
      'INSERT INTO deployment_previews (site_id, deployment_id, preview_path, full_preview_path, favicon_path, mime_type) VALUES ($1, $2, $3, $4, $5, $6) RETURNING *',
      [siteId, deploymentId, filePath, fullPreviewPath, faviconPath, mimeType]
    );
