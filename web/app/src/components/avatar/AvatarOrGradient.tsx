...
export const AvatarOrGradient: FC<{
    src: string;
    hash: string;
    className?: string;
    style?: CSSProperties;
}> = ({ src, hash, className, style }) => {
    const svg = useMemo(() => generateSVG(`team-${hash}`), [hash]);
    const [useGradient, setUseGradient] = useState(!src);
        ...
        <div className={className} style={style}>
            {(useGradient && (
                ...
                <img
                    src={src}
                    onError={(v) => {
                        console.log(v.currentTarget.src);
                        setUseGradient(true);
                    }}
