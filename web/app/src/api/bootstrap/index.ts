...
export const getCanBootstrap = () => queryOptions({
    ...
    staleTime(query) {
        if (query.state.data?.can_bootstrap) { return 1000 * 5 }
        return 1000 * 60 * 60 * 24
    }
});

export const bootstrapPreflight = async () => {
    const { can_bootstrap } = await queryClient.ensureQueryData(getCanBootstrap());
